...
export class InstructionOptions extends React.Component {
  ...
  render() {
    const instructionOptions = this.props.instruction.map((instruction, index) => {
      ...
      return <li key={index}><a className={instructionClassName} alt={instructionClassName} index={index} onClick={this.handleClick.bind(this, index)} tabindex="0">{option}</a></li>
